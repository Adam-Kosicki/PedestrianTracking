...
def extract_roi_from_video(video_path, regions):
    ...
    points = [(0, 0), (0, video_info.height - 1), (video_info.width-1, video_info.height - 1), (video_info.width-1, 0)]
...
def start_detection():
    ...
    region_names = "area" # Can be changed to denote various areas in the map
    num_regions = len(region_names.split(","))

    # Validate inputs
    if not video_path or not target_dir or not region_names or not str(num_regions).isdigit():
